  ...
  shell.innerHTML = `
    <header class="topbar">
      <div>
        ...
        <p>Backend-mode review shell with fixture fallback. Backend owns state. Engine owns ranking. This app renders artifacts, records structured edits, previews release, hands off Canvas signing, and asks the backend to release. The local sign control is audit metadata only until Canvas legal signature evidence is proven.</p>
      </div>
  ...
  queuePane.appendChild(Queue({
    ...
    onOpen: async (patientId) => {
      ...
      if (state.activeCase?.patient_packet?.patient_id === patientId) {
        ...
        state.workflowStatus = reviewPatient ? 'Backend review started.' : 'Fixture fallback review opened locally.';
        workflowEvent(
